...
const User = ({ avatar, isFollowed, name }: Props) => {
	return (
		<div className="bg-dark-cloud w-96 px-4 py-3 rounded-lg mb-2 flex items-center justify-between">
			...
			{isFollowed ? (
				<button className="bg-primary-purple text-sm border-primary-purple rounded-lg px-2 py-1 font-bold focus:outline-none focus:ring-2">
					Following
				</button>
			) : (
				<button className="border rounded-lg px-2	text-sm py-1 font-bold focus:outline-none focus:ring-2">Follow</button>
			)}
		</div>
	);
};
